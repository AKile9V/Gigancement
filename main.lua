...
function CreateCheckbox(option, label, parent, tooltip, new, subCB)
    ...
    local function UpdateOption(value, clicked)
        ...
        if checkBox.option == "enableUpgradedRaidFrames" then
            ...
            ToggleEventRegister("PLAYER_DIFFICULTY_CHANGED", settingsDB.enableUpgradedRaidFrames)
            ToggleEventRegister("PLAYER_ROLES_ASSIGNED", settingsDB.enableUpgradedRaidFrames)
            UpgradeRaidFrames()
            return
        end
...
function settingsInterface:Initialize()
    ...
    self.skipRoleCheck = CreateCheckbox("enableSkipRoleCheck", "Auto Role Check and\nSkip the Note Popup", self.scrollChild, "Automatically accept the role check popup when a party leader is applying your group to a Raid/M+ group and skip the note popup. If you want to sign up with a note, hold |cff00FF00Shift|r when signing up.")
    ...
    self.castTimePosition = CreateDropDownMenu("castbarTextPosition", "Castbar Text Position", self.scrollChild, 150, "Select the position for cast bar text.", 0, 1)
    ...
    self.characterILVLInfo = CreateCheckbox("enableCharacterILVLInfo", "Show Item Level", self.scrollChild, "Show item level on the Character and Inspect frames for each equipment slot.")
    self.characterILVLInfo:SetPoint("TOPLEFT", self.classColorILVL, "BOTTOMLEFT", 0, -10)
    self.characterEnchantsInfo = CreateCheckbox("enableCharacterEnchantsInfo", "Show Enchants", self.scrollChild, "Show enchants on the Character and Inspect frames for each equipment slot.")
    self.characterEnchantsInfo:SetPoint("TOPLEFT", self.characterILVLInfo, "BOTTOMLEFT", 0, -10)
    self.characterGemsInfo = CreateCheckbox("enableCharacterGemsInfo", "Show Gems", self.scrollChild, "Show gems on the Character and Inspect frames for each equipment slot.")
...
settingsInterface:RegisterEvent("PLAYER_DIFFICULTY_CHANGED")
settingsInterface:RegisterEvent("PLAYER_ROLES_ASSIGNED")
